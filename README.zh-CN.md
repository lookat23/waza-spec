...
它不是 `agent-spec` 的替代品。它从 Markdown 优先开始：模板、一个 skill、示例和一套有纪律的工作流。如果这套工作流在真实项目中证明有用，之后可以再加入 CLI。
...
`agent-spec` 是由合约驱动的验证体系，包含 CLI、解析器、linter、生命周期门禁和 guard。

`waza-spec` 是一种 Waza 风格的生命周期格式。它之后可以和 `agent-spec` 集成，但 v0.1 有意避免二进制依赖。

当你需要确定性的机器验证时，使用 `agent-spec`。当主要问题是让较大型的 Waza 驱动任务在计划、实现、审查和收尾之间保持一致时，使用 `waza-spec`。
